...
import ViewbrokerDetails from "./view_broker_details";
import ConfirmDeleteAgent from "../Agentmanagement/confirm_delete_agent";
import ConfirmDeletebroker from "./confirm_delete_broker";
...
const licenceBadge = (licence) => {
  const styles = {
    Valid: "bg-[#DCFCE7] text-[#016630]",
    Expired: "bg-[#FCE7DC] text-[#D02F2D]",
    "Under Review": "bg-[#FEF9C2] text-[#894B00]",
  };
  return (
    <span
      className={`inline-flex items-center justify-center px-3 py-1 rounded-full text-xs font-medium font-[Poppins] whitespace-nowrap ${styles[licence] || "bg-gray-100 text-gray-600"}`}
    >
      {licence}
    </span>
  );
...
function SearchFilterBar({ search, setSearch, statusFilter, setStatusFilter, roleFilter, setRoleFilter }) {
  return (
    <div className="bg-card rounded-[14px] shadow-[0_4px_6px_-1px_rgba(0,0,0,0.10),0_2px_4px_-2px_rgba(0,0,0,0.10)] p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex-1 flex items-center gap-2.5 bg-[#F3F3F5] rounded-lg px-4 py-2.5">
          ...
          <input
            ...
            className="flex-1 bg-transparent text-sm font-[Poppins] text-foreground placeholder-[#717182] outline-none min-w-0"
          />
        </div>
        ...
        <div className="flex gap-2 flex-shrink-0">
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none bg-[#F3F3F5] rounded-lg px-4 pr-9 py-2.5 text-sm font-[Poppins] text-foreground outline-none cursor-pointer min-w-[140px] h-[40px]"
            >
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none bg-[#F3F3F5] rounded-lg px-4 pr-9 py-2.5 text-sm font-[Poppins] text-foreground outline-none cursor-pointer min-w-[130px] h-[40px]"
            >
...
function BrokerageTable({ users, setbroker_details ,setdelete_broker}) {
  return (
    <div className="bg-card rounded-[12px] border border-[#E6E7E6] overflow-hidden">
            ...
            {users.length === 0 ? (
              ...
              users.map((user, i) => (
                ...
                >
                  ...
                  {/* BROKERAGE */}
                  <td className="px-6 py-3 text-center text-sm font-medium text-[var(--brand-dark))]">
                    {user.brokerage}
                  </td>
                  ...
                  <td className="px-6 py-3">
                    <div className="flex items-center justify-center gap-4">
                      ...
                      {/* VIEW */}
                      <button
                        onClick={() => setbroker_details(true)}
                      ...
                      {/* DELETE */}
                      <button onClick={() => setdelete_broker(true)}
...
export default function Brokeragemanagement() {
  ...
  const [broker_details, setbroker_details] = useState(false);
  const [delete_broker, setdelete_broker] = useState(false)

  const [search, setSearch] = useState("");
  const [statusFilter, setStatusFilter] = useState("");
  const [roleFilter, setRoleFilter] = useState("");

  const filtered = Broker.filter((u) => {
    const matchSearch =
      !search ||
  ...
  return (
    <div className=" bg-background font-[Poppins]">
      <div className="w-full mx-auto px-2 sm:px-2 lg:px-2 py-2 sm:py-2">
        {/* Page Header */}
        {/* <div className="mb-6">
          <h1 className="text-2xl sm:text-3xl font-semibold text-foreground font-[Poppins]">
            User Management
          </h1>
        ...
        {/* Brokerage Table */}
        <BrokerageTable users={filtered} setbroker_details={setbroker_details} setdelete_broker={setdelete_broker}/>

        {
          broker_details && (
            <ViewbrokerDetails onClose={() => setbroker_details(false)} />
          )
        }
        ...
        {
          delete_broker && (
            <ConfirmDeletebroker onCancel={() => setdelete_broker(false)} />
          )
        }
        ...
        <div className="mt-3 px-1">
          <p className="text-sm text-[#717182] font-[Poppins]">
            Showing {filtered.length} of {Broker.length} users
          </p>
        </div>
      </div>
    </div>
