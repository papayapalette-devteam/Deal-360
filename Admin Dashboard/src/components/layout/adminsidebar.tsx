          ...
          {menuItems.map((item) => {
            ...
            return (
              <NavLink
                ...
                onClick={onItemClick}
                className={cn(
                  "flex font-sans items-center gap-3 px-3 xl:py-[1rem] lg:py-3 sm:text-[1rem] 2xl:text-lg rounded-lg text-lg font-medium transition-colors no-underline",
                  isActive
...
export default function Adminsidebar({ open, onOpenChange }) {
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex w-60 lg:w-72 xl:w-80 2xl:w-[23.8125rem] min-h-screen bg-card flex-col">
        <SidebarContent />
      </aside>
