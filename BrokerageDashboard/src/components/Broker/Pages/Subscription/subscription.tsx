...
export default function Subscription() {
  ...
  const handleCancelAnyway = () => {
    console.log("Cancel anyway");
    closeAllModals();
  };

  const handleConfirmFreeze = (date: string) => {
    console.log("Confirm freeze for", date);
    closeAllModals();
  };
  ...
  const cardClass = (style) => {
    ...
    if (style === 'gray') return 'rounded-xl bg-[var(--form-bg)] shadow-[0_0_10px_0_rgba(0,0,0,0.10)] relative';
    return 'border border-[#E5E7EB] bg-white rounded-xl shadow-[0_1px_2px_0_rgba(0,0,0,0.05)] relative';
  };

  const buttonClass = (style) => {
    if (style === 'coral') return 'flex items-center justify-center gap-2 w-full py-[0.6875rem] px-4 rounded-lg bg-[var(--form-primary)] text-white font-medium text-sm';
    if (style === 'mint') return 'flex items-center justify-center w-full py-[0.6875rem] px-4 rounded-lg bg-[var(--form-secondary)] text-[#0A0A0A] font-medium text-sm';
    return 'flex items-center justify-center w-full py-[0.6875rem] px-4 rounded-lg bg-white text-[#0A0A0A] font-medium text-sm';
  };
  ...
  /* ─── render ─── */
  return (
    <div className="min-h-screen bg-[var(--form-bg)] py-6 px-4 md:px-6 lg:px-8" style={{ fontFamily: "'Inter', sans-serif" }}>
      <div className="max-w-[86.25rem] mx-auto flex flex-col gap-8">

        {/* ── 1. Header ── */}
        <div className="flex flex-col items-start gap-2.5">
          <div className="flex items-center gap-2 py-[0.5625rem] px-8 rounded-lg bg-[var(--form-primary)]">
            <span className="text-white text-sm font-medium leading-6">Renewal date: 10 Mar 2026</span>
          </div>
        </div>

        <div className="flex justify-center">
          <button className="py-[0.6875rem] px-[5.3125rem] rounded-lg bg-[var(--form-secondary)] text-[#0A0A0A] text-sm font-medium leading-6 whitespace-nowrap">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-6">
          {plans.map((plan) => (
            <div key={plan.id} className={cardClass(plan.cardStyle)}>
              ...
              {/* Current Plan badge */}
              {plan.isCurrent && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 flex items-center justify-center px-6 py-[0.1875rem] rounded-lg border border-black/10 bg-[var(--form-primary)] overflow-hidden z-10 whitespace-nowrap">
              ...
              <div className="p-6 flex flex-col h-full">
                ...
                {/* Plan name */}
                <h3 className="text-[#0A0A0A] text-[0.9375rem] font-bold leading-7 mb-1">{plan.name}</h3>
                  ...
                  {plan.priceUnit ? (
                    <>
                      <span
                        ...
                        style={{ fontFamily: "'Poppins', sans-serif", fontSize: '2.25rem' }}
                      >
                        {plan.price}
                      </span>
                      <span
                    ...
                    <span
                      ...
                      style={{ fontFamily: "'Inter', sans-serif", fontSize: '1.625rem', lineHeight: '36px' }}
                    >
                      {plan.price}
                    </span>
                  )}
                ...
                {/* Description */}
                <p className="text-[#6B7280] text-xs leading-[1.375rem] mb-4 min-h-[2.75rem]">{plan.desc}</p>

                {/* Credits pill */}
                <div className="flex items-center justify-between px-3 py-[0.8125rem] rounded-lg bg-[#FFEAE8] mb-3">
                  <span className="text-[#374151] text-xs font-medium leading-5">Monthly Credits  Included</span>
                  <span className="text-[#4B5563] text-[0.9375rem] font-bold leading-7">{plan.credits}</span>
                </div>

                {/* Includes label */}
                <p className="text-[#9CA3AF] text-[0.625rem] font-semibold leading-4 mb-2">Includes</p>
        ...
        <div className="flex flex-col gap-3 p-2 rounded-2xl bg-[#F6F6F6]">
          ...
          <button onClick={() => openModal("downgrade")} className="flex items-center justify-center py-[0.8125rem] px-8 rounded-lg bg-white w-full">
            <span className="text-black font-medium text-base" style={{ fontFamily: "'Manrope', sans-serif" }}>Downgrade plan</span>
          </button>
          <button onClick={() => openModal("freeze")} className="flex items-center justify-center py-[0.8125rem] px-8 rounded-lg bg-white w-full">
            <span className="text-black font-medium text-base" style={{ fontFamily: "'Manrope', sans-serif" }}>Freeze for 1 month (Travel mode)</span>
          </button>
          <button onClick={() => openModal("cancel")} className="flex items-center justify-center py-[0.8125rem] px-8 rounded-lg bg-white w-full">
            <span className="text-black font-medium text-base" style={{ fontFamily: "'Manrope', sans-serif" }}>Cancel subscription anytime</span>
          </button>
        </div>

        {/* ── 4. Available Credits ── */}
        <div className="rounded-[0.875rem] border-2 border-[var(--form-primary)] bg-[rgba(248,131,121,0.12)] p-8">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex items-center gap-3">
                ...
                <span
                  ...
                  style={{ fontFamily: "'Manrope', sans-serif", fontSize: '3rem', lineHeight: '48px' }}
                >
                  156
                </span>
              </div>
            ...
            <div className="flex flex-col items-stretch sm:items-end gap-2 w-full sm:w-auto">
              <button className="flex items-center justify-center gap-2 py-2 px-4 rounded-lg bg-[var(--form-secondary)] h-[2.625rem] whitespace-nowrap">
                <PlusIcon />
                <span className="text-black text-sm font-normal" style={{ fontFamily: "'Poppins', sans-serif" }}>Buy More Credits</span>
              </button>
              <button className="flex items-center justify-center py-[0.5625rem] px-8 rounded-lg bg-[var(--form-primary)] h-12 whitespace-nowrap">
              ...
              >
                ...
                {pkg.popular && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 flex items-center justify-center px-3 py-1 rounded-full bg-[var(--form-secondary)] shadow-md whitespace-nowrap">
                    <span className="text-[#4B4B4B] text-[0.625rem] font-bold leading-4">Most popular</span>
                  </div>
                )}
                ...
                {/* Credits label */}
                <p className="text-[#111827] text-[0.9375rem] font-bold leading-7 text-center mb-2">
                  {pkg.credits} Credits
                </p>
                ...
                {/* Button */}
                {pkg.popular ? (
                  <button className="w-full py-[0.6875rem] px-4 rounded-lg bg-[var(--form-primary)] text-white text-sm font-medium text-center">
                    Purchase Pack
                  </button>
                ) : (
        ...
        {/* ── 6. Usage History ── */}
        <div className="rounded-[0.875rem] border border-black/10 bg-white flex flex-col gap-8 p-6">
          ...
          <div className="flex flex-col gap-3">
            {usageHistory.map((item, i) => (
              <div
                key={i}
                className="flex items-center justify-between px-4 py-5 rounded-[0.625rem]"
              >
                {/* Left: icon + text */}
