...
const MintBadge = ({ children }) => (
  <div className="flex h-[2rem] items-center justify-center gap-4 rounded-[0.5rem] border border-[var(--form-secondary)] px-3 overflow-hidden">
    <span className="font-poppins text-[0.875rem] font-normal leading-[1.25rem] text-[#0F172A] whitespace-nowrap">
      {children}
    </span>
  </div>
);
...
const LocationBadge = ({ children }) => (
  <div className="flex items-center gap-[0.5rem] rounded-[0.5rem]  px-[1.5rem] py-[0.5rem]" style={{border:"1px solid #F88379"}}>
    <LocationPinIcon />
    <span className="font-poppins text-[0.875rem] font-normal leading-[1.25rem] text-[#0F172A] whitespace-nowrap">
      {children}
    </span>
