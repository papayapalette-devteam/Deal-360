...
function ToggleSwitch({ enabled, onChange }) {
  return (
    <button
      ...
      className="relative flex-shrink-0 w-[2.66rem] h-[1.33rem] rounded-full transition-colors duration-200 focus:outline-none"
      style={{
    ...
    >
      <span
        className="absolute top-[0.22rem] w-[0.88rem] h-[0.88rem] rounded-full bg-white shadow transition-all duration-200"
        style={{
          left: enabled ? "calc(100% - 1.11rem)" : "0.22rem",
        }}
      />
    </button>
  );
...
export default function NotificationPreferences() {
  ...
  return (
    <div
      className="rounded-[0.55rem]  bg-white dark:bg-card shadow-sm overflow-hidden"
