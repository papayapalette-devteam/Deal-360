...
function TransactionTable({ title, data }) {
  return (
    <div className="bg-white rounded-[0.44rem] p-[0.88rem]">
      <div className="flex flex-col gap-[0.88rem]">
        <h4 className="text-[0.88rem] font-medium text-[#0A0A0A] font-poppins">{title}</h4>
        <div className="rounded-[0.66rem] border border-[#E6E7E6] overflow-hidden">
          <table className="w-full border-collapse">
            <thead>
              <tr>
                <th className="h-[2.44rem] px-0 text-center text-[0.77rem] font-normal text-black border-r border-b border-[#E6E7E6] bg-[rgba(248,131,121,0.10)] font-poppins">Year</th>
                <th className="h-[2.44rem] px-0 text-center text-[0.77rem] font-normal text-black border-r border-b border-[#E6E7E6] bg-[rgba(248,131,121,0.10)] font-poppins">Residential</th>
                <th className="h-[2.44rem] px-0 text-center text-[0.77rem] font-normal text-black border-r border-b border-[#E6E7E6] bg-[rgba(248,131,121,0.10)] font-poppins">Commercial</th>
                <th className="h-[2.44rem] px-0 text-center text-[0.77rem] font-normal text-black border-b border-[#E6E7E6] bg-[rgba(248,131,121,0.10)] font-poppins">Total</th>
              </tr>
            </thead>
            <tbody>
              {data.map((row, i) => (
                <tr key={row.year} className={i < data.length - 1 ? "border-b border-[#E6E7E6]" : ""}>
                  <td className="h-[4rem] px-[0.66rem] text-center text-[0.77rem] font-medium text-black border-r border-[#E6E7E6] font-poppins">{row.year}</td>
                  <td className="h-[4rem] px-[0.66rem] text-center text-[0.77rem] font-medium text-black border-r border-[#E6E7E6] font-poppins">{row.residential}</td>
                  <td className="h-[4rem] px-[0.66rem] text-center text-[0.77rem] font-medium text-black border-r border-[#E6E7E6] font-poppins">{row.commercial}</td>
                  <td className="h-[4rem] px-[0.66rem] text-center text-[0.77rem] font-medium text-black font-poppins">{row.total}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function ToggleSwitch({ value, onChange }) {
  return (
    <div className="flex p-[0.44rem] gap-[0.22rem] bg-[var(--form-section-bg)] rounded-[0.44rem] w-[7.61rem]">
      {["On", "Off"].map(opt => (
        <button
          key={opt}
          type="button"
          onClick={() => onChange(opt.toLowerCase())}
          className={`flex-1 py-[0.66rem] rounded-[0.44rem] text-[0.66rem] font-medium text-[var(--form-text)] font-poppins transition-colors ${value === opt.toLowerCase() ? "bg-white shadow-sm" : ""}`}
        >
          {opt}
        </button>
      ))}
...
function SegmentedControl({ options, value, onChange }) {
  return (
    <div className="flex p-[0.44rem] gap-[0.22rem] bg-[var(--form-section-bg)] rounded-[0.44rem]">
      {options.map(opt => (
        <button
          key={opt.value}
          type="button"
          onClick={() => onChange(opt.value)}
          className={`px-[0.88rem] py-[0.66rem] rounded-[0.44rem] text-[0.66rem] font-medium text-[var(--form-text)] font-poppins transition-colors whitespace-nowrap ${value === opt.value ? "bg-white shadow-sm" : ""}`}
        >
          {opt.label}
        </button>
      ))}
...
function FormField({ label, name, value, onChange, placeholder = "" }) {
  return (
    <div className="flex flex-col gap-[0.44rem] w-full">
      <label className="text-[0.88rem] font-medium text-[#434C45] font-poppins">{label}</label>
      <input
        type="text"
        value={value}
        onChange={e => onChange(name, e.target.value)}
        placeholder={placeholder}
        className="h-[2.66rem] px-[0.66rem] rounded-[0.22rem] border border-[#C0C3C1] bg-white text-[var(--text-grey)] text-[0.88rem] font-poppins outline-none focus:border-[var(--form-primary)] transition-colors"
      />
    </div>
  );
}
...
export default function EditProfile() {
  ...
  return (
    <div className="min-h-screen bg-[var(--Colors-for-white-theme-White)] p-0 md:p-0 font-poppins">
          <div className="w-full p-[0.44rem]">
      <button
        type="button"
        className="flex h-[3.33rem] w-full items-center justify-between rounded-[0.33rem] bg-white px-[0.88rem] shadow-[0_0.22rem_0.33rem_0_rgba(0,0,0,0.10)] hover:shadow-[0_0.33rem_0.55rem_0_rgba(0,0,0,0.12)] transition-shadow duration-200"
      >
        {/* Left: icon + label */}
        <div className="flex items-center gap-[0.55rem]">
          ...
          {/* Label */}
          <span className="font-poppins font-bold text-[0.77rem] leading-[120%] text-[#F88379]">
            Profile
          </span>
        </div>
      ...
      <div className="w-full  flex flex-col lg:flex-row gap-[1.33rem] items-stretch mt-[0.88rem]">


        {/* ══════════════════════════════════
            LEFT PANEL — Bio / Profile Preview
            ══════════════════════════════════ */}
        <div className="w-full lg:w-[22.77rem] xl:w-[23.33rem] flex-shrink-0 bg-white rounded-[0.44rem] shadow-[0_0.22rem_0.33rem_rgba(0,0,0,0.12)] overflow-hidden border border-[#E6E7E6]">

          {/* Gradient banner */}
          <div className="relative">
            <div
              className="h-[8.33rem] w-full rounded-t-[0.44rem]"
            ...
            <div className="absolute left-1/2 -translate-x-1/2 bottom-0 translate-y-1/2">
              <div className="relative w-[7.22rem] h-[7.22rem]">
                <img
                  src="https://api.builder.io/api/v1/image/assets/TEMP/ab98ed6f04d865f8372187d5b599d544c540d58b?width=320"
                  alt="Natasha Howe"
                  className="w-full h-full rounded-full border-[0.22rem] border-[#F7F8F8] shadow-[0_0.22rem_0.83rem_rgba(0,0,0,0.10)] object-cover"
                />
                <button
                  type="button"
                  className="absolute bottom-[0.22rem] right-[0.22rem] w-[1.77rem] h-[1.77rem] bg-[var(--form-primary)] rounded-full border-[0.11rem] border-[var(--form-bg)] flex items-center justify-center shadow-md"
                >
          ...
          <div className="mt-[4.33rem] px-[1.33rem] pb-[1.77rem] flex flex-col gap-[1.11rem]">

            {/* Name */}
            <h2 className="text-center text-[1.33rem] font-semibold text-[var(--form-primary)] font-poppins">Natasha Howe</h2>

            {/* Bio */}
            <div className="flex flex-col gap-[0.44rem]">
              <span className="text-[0.88rem] font-medium text-[#434C45] font-poppins">Bio</span>
              <p className="text-[0.83rem] text-[var(--text-grey)] leading-[1.6] font-poppins font-normal">
            ...
            <p className="text-[0.77rem] font-medium text-[var(--form-primary)] leading-[1.4] font-poppins">
            ...
            <div className="flex flex-col gap-[0.44rem]">
              <span className="text-[0.77rem] font-medium text-[var(--form-primary)] font-poppins">Display Transaction History on Public Profile</span>
        ...
        {/* ══════════════════════════════════
            ...
            ══════════════════════════════════ */}
        <div className="flex-1 min-w-0 bg-[var(--form-bg)] rounded-[0.44rem] border border-[#E6E7E6] shadow-[0_0.22rem_0.44rem_rgba(0,0,0,0.12)] p-[1.33rem] flex flex-col gap-[1.33rem]">

          {/* ── PUBLIC INFORMATION ── */}
          <div className="flex flex-col gap-[0.22rem]">
            <h1 className="text-[1.77rem] font-bold text-[var(--base-black)] font-manrope leading-[1.2]">Public Information</h1>
            <p className="text-[0.88rem] font-normal text-[var(--text-grey)] font-poppins">Manage your personal information and profile details.</p>
          </div>

          {/* Gender */}
          <div className="flex items-center gap-[1.33rem]">
            {[{ value: "male", label: "Male" }, { value: "female", label: "Female" }].map(g => (
              <div key={g.value} className="flex items-center gap-[0.44rem] cursor-pointer" onClick={() => setGender(g.value)}>
                <span className="text-[0.77rem] font-medium text-[#434C45] font-poppins">{g.label}</span>
                <div className="w-[1rem] h-[1rem] rounded-full border border-[#434C45] flex items-center justify-center flex-shrink-0 p-[0.16rem]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-[1.33rem]">
          ...
          {/* Expertise */}
          <div className="flex flex-col gap-[0.88rem]">
            <span className="text-[0.88rem] font-medium text-[#0A0A0A] font-poppins">Expertise</span>
          ...
          {/* Coverage Areas */}
          <div className="flex flex-col gap-[0.88rem] p-[0.88rem] rounded-[0.44rem] border border-transparent">
            <span className="text-[0.88rem] font-medium text-[#0A0A0A] font-poppins">Coverage Areas</span>

            {/* Area input row */}
            <div className="flex items-center gap-[0.44rem] px-[0.66rem] py-[0.44rem] bg-[#EFEFEF] rounded-[0.44rem]">
              <LocationPin />
              <input
                type="text"
                value={newArea}
                ...
                className="flex-1 bg-transparent text-[0.77rem] text-[#7C7C7C] outline-none placeholder:text-[#7C7C7C] font-poppins min-w-0"
              />
              <button
                type="button"
                onClick={addArea}
                className="h-[2.5rem] px-[1.33rem] bg-[var(--form-primary)] text-[var(--form-bg)] text-[0.77rem] font-semibold font-poppins rounded-[0.33rem] flex-shrink-0 hover:bg-[#e8726a] transition-colors"
              >
                Add
              </button>
            </div>
            ...
            {/* Area tags */}
            <div className="flex flex-wrap gap-[0.66rem]">
              {areas.map(area => (
                <div
                  key={area.id}
                  className="flex items-center gap-[0.44rem] px-[1.11rem] py-[0.44rem]   rounded-[0.44rem] cursor-pointer hover:bg-[#fff5f4] transition-colors group"
                  onClick={() => removeArea(area.id)}
                  title="Click to remove"
                  style={{border:"1px solid #F88379"}}
                >
                  <LocationPin />
                  <span className="text-[0.77rem] font-normal text-[#0F172A] font-poppins">{area.name}</span>
          ...
          {/* ── PRIVATE INFORMATION ── */}
          <div className="flex flex-col gap-[1.33rem]">
            <h2 className="text-[1.77rem] font-bold text-[var(--base-black)] font-manrope leading-[1.2]">Private Information</h2>
          ...
          <div className="flex flex-col gap-[1.11rem]">
            <h3 className="text-[1.33rem] font-semibold text-[var(--base-black)] font-poppins">Billing Details</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-[1.33rem]">
              ...
              {/* Upload KYC */}
              <div className="flex flex-col gap-[0.44rem]">
                <label className="text-[0.88rem] font-medium text-[#434C45] font-poppins">Upload KYC Doc</label>
                <label className="h-[2.66rem] px-[0.66rem] rounded-[0.22rem] border border-[#C0C3C1] bg-white flex items-center justify-end cursor-pointer hover:border-[var(--form-primary)] transition-colors">
          ...
          {/* Action buttons */}
          <div className="flex flex-wrap justify-end items-center gap-[0.88rem] pt-[0.44rem]">
            <button
              type="button"
              className="h-[2.5rem] px-[1.33rem] border border-[var(--text-grey)] rounded-[0.33rem] text-[0.77rem] font-medium text-[var(--text-grey)] font-poppins hover:bg-gray-100 transition-colors"
            >
              Discard Changes
            </button>
            <button
              type="button"
              className="h-[2.5rem] w-[7.88rem] bg-[var(--form-primary)] rounded-[0.33rem] text-[0.77rem] font-semibold text-[var(--form-bg)] font-poppins hover:bg-[#e8726a] transition-colors"
            >
              Save Changes
            </button>
          </div>
