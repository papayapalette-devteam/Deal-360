...
const WhatsAppIcon = () => (
  <div className="relative w-[1.11rem] h-[1.11rem] flex-shrink-0">
    <svg style={{ width: "1.11rem", height: "1.11rem" }} viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <svg
      style={{ position: "absolute", top: "0.22rem", left: "0.22rem", width: "0.66rem", height: "0.66rem" }}
...
const DropdownChevron = () => (
  <svg style={{ width: "0.55rem", height: "0.27rem" }} viewBox="0 0 10 5" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default function AgentProfilePage() {
  ...
  return (
    <div className="min-h-screen bg-background p-0 sm:p-0 lg:p-0">
      <div className="mx-auto w-full flex flex-col gap-6">
        ...
        <div className="flex justify-end">
          <button onClick={()=>navigate('edit-profile')} className="flex items-center gap-[0.55rem] rounded-[0.44rem] bg-[var(--form-secondary)] px-[0.88rem] py-[0.55rem] hover:opacity-90 transition-opacity">
            <EditIcon />
            <span className="font-manrope text-[0.88rem] font-normal leading-[1.11rem] text-black">
              Edit Profile
            </span>
          </button>
        </div>
        ...
        {/* Agent Profile Card */}
        <div className="relative rounded-[0.55rem] border border-black/10 bg-white shadow-[0_1px_3px_0_rgba(0,0,0,0.10),0_1px_2px_-1px_rgba(0,0,0,0.10)] overflow-visible">

          {/* Gradient Banner */}
          <div
            className="h-[6.5rem] rounded-t-[0.44rem]"
            style={{ background: "linear-gradient(90deg, rgba(255,159,151,0.60) 0%, rgba(183,255,231,0.50) 100%)" }}
          />

          {/* Avatar – overlaps the banner */}
          <div
            className="absolute rounded-full border-[0.18rem] border-white bg-gray-100 overflow-hidden shadow-[0_10px_15px_-3px_rgba(0,0,0,0.10),0_4px_6px_-4px_rgba(0,0,0,0.10)]"
            style={{
              top: "4.11rem",
              left: "2rem",
              width: "7.11rem",
          ...
          {/* Card Body */}
          <div className="p-[1.33rem] pt-[1.11rem]">

            {/* Profile Info Section */}
            <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">

              {/* Left: Name + details (with left offset for avatar on desktop) */}
              <div className="flex flex-col gap-[0.33rem] lg:pl-[8.44rem] pt-[0.44rem] lg:pt-0 mt-[3.55rem] lg:mt-0">
                {/* On mobile, avatar doesn't overlap, so we push content down with mt-16 */}
                <h1 className="font-sans text-[1.22rem] sm:text-[1.44rem] lg:text-[1.55rem] font-semibold leading-[120%] text-[var(--base-black)]">
                  Natasha Howe
                </h1>

                <div className="flex items-center gap-[0.44rem] flex-wrap">
                  <span className="font-poppins text-[0.77rem] sm:text-[0.88rem] font-medium leading-[120%] text-[var(--form-secondary)]">
                    Barnes International Realty-Dubai
                  </span>
                  <DropdownChevron />
                </div>

                <span className="font-poppins text-[0.72rem] sm:text-[0.77rem] font-normal leading-[120%] text-[#999]">
                  Senior Property Consultant
                </span>

                <span className="font-poppins text-[0.77rem] sm:text-[0.88rem] font-medium leading-[120%] text-[var(--base-black)]">
                  34 Active Properties
                </span>
              </div>

              {/* Right: Contact Buttons */}
              <div className="flex items-center gap-[0.44rem] flex-wrap lg:flex-nowrap lg:self-end lg:pb-[0.22rem] lg:ml-auto">
                <button className="flex items-center gap-[0.44rem] h-[2.55rem] px-[0.88rem] sm:px-[1.33rem] rounded-[0.44rem] bg-[var(--form-secondary)] shadow-[0_0_10px_0_rgba(255,255,255,0.25)_inset] hover:opacity-90 transition-opacity">
                  <MailIcon />
                  <span className="font-manrope text-[0.77rem] sm:text-[0.88rem] font-medium leading-[120%] text-black">Email</span>
                </button>
                <button className="flex items-center gap-[0.44rem] h-[2.55rem] px-[0.88rem] sm:px-[1.33rem] rounded-[0.44rem] bg-[var(--form-secondary)] shadow-[0_0_10px_0_rgba(255,255,255,0.25)_inset] hover:opacity-90 transition-opacity">
                  <CallIcon />
                  <span className="font-manrope text-[0.77rem] sm:text-[0.88rem] font-medium leading-[120%] text-black">Call</span>
                </button>
                <button className="flex items-center gap-[0.44rem] h-[2.55rem] px-[0.88rem] sm:px-[1.33rem] rounded-[0.44rem] bg-[var(--form-secondary)] shadow-[0_0_10px_0_rgba(255,255,255,0.25)_inset] hover:opacity-90 transition-opacity">
                  <WhatsAppIcon />
                  <span className="font-manrope text-[0.77rem] sm:text-[0.88rem] font-medium leading-[120%] text-black">WhatsApp</span>
                </button>
              </div>
            </div>

            {/* Badge Row */}
            <div className="flex items-center gap-[0.44rem] mt-[0.88rem] lg:pl-[8.44rem] flex-wrap">
              <button className="flex items-center justify-center h-[2.66rem] px-[0.88rem] rounded-[0.44rem] bg-[var(--form-secondary)] min-w-[6.66rem] hover:opacity-90 transition-opacity">
                <span className=" text-[0.77rem] font-medium leading-[120%] text-[var(--base-black)]">Agent</span>
              </button>
              <button className="flex items-center justify-center gap-[0.44rem] h-[2.66rem] px-[0.88rem] rounded-[0.44rem] bg-[rgba(248,131,121,0.60)] min-w-[10rem] hover:opacity-90 transition-opacity">
                <PhoneIcon />
                <span className=" text-[0.77rem] font-medium leading-[120%] text-[var(--base-black)]">Responsive</span>
              </button>
            </div>

            {/* Divider */}
            <div className="mt-[1.33rem] mb-[0.88rem] h-px bg-black/5" />
            ...
            <div className="flex flex-col gap-[1.33rem]">
              ...
              <div className="flex flex-col gap-[0.44rem]">
                <h2 className=" text-[1rem] font-medium leading-[120%] text-black font-poppins">
                  About Natasha
                </h2>
                <p className="font-poppins text-[0.77rem] font-normal leading-[162.5%] text-[#4A5565]">
                  {expanded ? BIO_FULL : BIO_SHORT}{" "}
                  <button
                    onClick={() => setExpanded(!expanded)}
                    className="font-arimo text-[0.88rem] font-normal leading-[1.33rem] text-[#00A680] hover:underline"
                  >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-[1.77rem] xl:gap-[3.55rem] items-start">
                <TrustSection />
                <SpecialtiesSection />
   ...
   <div className="mx-auto max-w-[77.7rem] flex flex-col gap-[1.33rem] mt-[0.88rem]">
                  <div className="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-1 gap-[1.77rem] xl:gap-[3.55rem] items-start">
                <ContactCard />
              </div>
              </div>
    </div>
