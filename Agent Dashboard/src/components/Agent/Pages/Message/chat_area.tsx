...
const PhoneIcon = () => (
  <svg style={{ width: "1.11rem", height: "1.11rem" }} viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <defs><clipPath id="phoneClip"><rect style={{ width: "1.11rem", height: "1.11rem" }} fill="white"/></clipPath></defs>
  </svg>
);
...
const SendPaperPlaneIcon = () => (
  <svg style={{ width: "1.38rem", height: "1.5rem" }} viewBox="0 0 25 27" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function MessageBubble({ message }) {
  return (
    <div className={`flex ${message.sent ? "justify-end" : "justify-start"}`}>
      <div
        className={`max-w-[85%] sm:max-w-[70%] lg:max-w-[22.22rem] px-4 pt-3 pb-3 rounded-[0.55rem] flex flex-col gap-1 ${
          ...
          <div className="flex items-center justify-between 2xl:justify-start 2xl:gap-6">
            ...
            <div className="flex items-center gap-2 ">
              ...
              <button
                ...
                className={`relative w-[2.44rem] h-[1.33rem] rounded-full transition-colors shrink-0 ${
                  autoReplay ? "bg-[var(--form-secondary)]" : "bg-gray-300"
                }`}
              >
                <span
                  className={`absolute top-[3px] w-[1rem] h-[1rem] rounded-full bg-white shadow transition-transform ${
  ...
  return (
    <div className="border-t border-[hsl(var(--chat-border))] bg-white shrink-0 px-4 py-4">
      <div className="flex items-center gap-3">
        {/* Attachment */}
        <button className="flex items-center justify-center w-[2rem] h-[2rem] rounded-[0.55rem] hover:bg-gray-100 transition-colors shrink-0">
          <AttachIcon />
        </button>
        ...
        <input
          ...
          className="flex-1 h-[2.77rem] px-4 rounded-[0.55rem] border border-[hsl(var(--chat-input-border))] font-manrope text-base text-[rgba(10,10,10,0.5)] placeholder:text-[rgba(10,10,10,0.5)] focus:outline-none focus:ring-1 focus:ring-primary bg-white"
        />

        {/* Send */}
        <button className="flex items-center justify-center w-[2.72rem] h-[2.77rem] rounded-[0.55rem] bg-[var(--form-secondary)] hover:bg-primary/90 transition-colors shrink-0">
          <SendPaperPlaneIcon />
        </button>
      </div>
    </div>
...
function ChatHeader({ conversation, onBack }) {
  return (
    <div className="flex items-center justify-between px-4 h-[4.05rem]  bg-white shrink-0" style={{borderBottom:"1px solid #F3F4F6"}}>
      {/* Left: Back (mobile) + Avatar + Name */}
      <div className="flex items-center gap-3">
        {/* Back button - mobile only */}
        <button
          onClick={onBack}
          className="md:hidden flex items-center justify-center w-[1.77rem] h-[1.77rem] rounded-lg hover:bg-gray-100 transition-colors"
        >
          <BackArrowIcon />
        </button>
        ...
        <div className="relative shrink-0">
          <img
            className="w-[2.66rem] h-[2.66rem] rounded-full object-cover"
            src={conversation.avatar}
            alt={conversation.name}
          />
          {conversation.online && (
            <span className="absolute bottom-0 right-0 w-[0.66rem] h-[0.66rem] rounded-full bg-primary border-2 border-white" />
          )}
        </div>
      ...
      {/* Right: Action icons */}
      <div className="flex items-center gap-2">
        <button className="flex items-center justify-center w-[2rem] h-[2rem] rounded-[0.55rem] hover:bg-gray-100 transition-colors">
          <PhoneIcon />
        </button>
        <button className="flex items-center justify-center w-[2rem] h-[2rem] rounded-[0.55rem] hover:bg-gray-100 transition-colors">
          <VideoIcon />
