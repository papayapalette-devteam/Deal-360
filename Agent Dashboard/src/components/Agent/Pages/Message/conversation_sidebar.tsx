          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2 shrink-0">
              {conversation.unread > 0 && (
                <span className="w-[1.11rem] h-[1.11rem] rounded-full bg-secondary flex items-center justify-center text-white text-xs font-arimo leading-4 shrink-0">
                  {conversation.unread}
                </span>
              )}
...
export default function ConversationSidebar({ conversations, activeId, onSelect, visible }) {
  ...
  return (
    <div
      className={`${
        visible ? "flex" : "hidden"
      } md:flex flex-col w-full md:w-[25.5rem] md:min-w-[25.5rem] h-full  bg-white shrink-0`}
    >
      {/* Search Bar */}
      <div className="px-4 pt-4 pb-[6px] shrink-0" style={{borderBottom:"1px solid #F3F4F6"}}>
        <div className="relative">
          ...
          <input
            ...
            className="w-full h-[2.33rem] pl-10 pr-4 rounded-[0.55rem] border border-[#D1D5DC] font-manrope text-base text-[rgba(10,10,10,0.5)] placeholder:text-[rgba(10,10,10,0.5)] focus:outline-none focus:ring-1 focus:ring-primary bg-white"
          />
        </div>
      </div>
