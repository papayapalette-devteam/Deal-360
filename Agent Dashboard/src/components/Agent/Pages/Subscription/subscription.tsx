...
export default function Subscription() {
  ...
  /* ─── inline SVG helpers ─── */
  const CheckIcon = () => (
    <svg style={{ width: "0.88rem", height: "0.88rem" }} viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" className="flex-shrink-0">
      <path d="M13.3307 4L5.9974 11.3333L2.66406 8" stroke="#7FFFD4" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  );

  const InfoCircleIcon = () => (
    <svg style={{ width: "1.33rem", height: "1.33rem" }} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="flex-shrink-0">
  ...
  const CreditIcon48 = () => (
    <svg style={{ width: "2.66rem", height: "2.66rem" }} viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" className="flex-shrink-0">
  ...
  const CreditIcon28White = () => (
    <svg style={{ width: "1.55rem", height: "1.55rem" }} viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg" className="flex-shrink-0">
  ...
  /* ─── card style helpers ─── */
  const cardClass = (style: string) => {
    if (style === 'highlighted') return 'border border-[#7FFFD4] bg-[rgba(127,255,212,0.10)] rounded-[0.66rem] relative';
    if (style === 'gray') return 'rounded-[0.66rem] bg-[var(--form-primary-foreground)] shadow-[0_0_10px_0_rgba(0,0,0,0.10)] relative';
    return 'border border-[#E5E7EB] bg-white rounded-[0.66rem] shadow-[0_1px_2px_0_rgba(0,0,0,0.05)] relative';
  };

  const buttonClass = (style: string) => {
    if (style === 'coral') return 'flex items-center justify-center gap-2 w-full py-[0.61rem] px-4 rounded-lg bg-[var(--form-primary)] text-white font-medium text-sm';
    if (style === 'mint') return 'flex items-center justify-center w-full py-[0.61rem] px-4 rounded-lg bg-[var(--form-secondary)] text-[#0A0A0A] font-medium text-sm';
    return 'flex items-center justify-center w-full py-[0.61rem] px-4 rounded-lg bg-white text-[#0A0A0A] font-medium text-sm';
  };

  const featureTextClass = (planId: string) => {
  ...
  /* ─── render ─── */
  return (
    <div className="min-h-screen bg-[var(--form-primary-foreground)] py-6 px-4 md:px-6 lg:px-[1.77rem]" style={{ fontFamily: "'Inter', sans-serif" }}>
      <div className="max-w-[1380px] mx-auto flex flex-col gap-8">

        {/* ── 1. Header ── */}
        <div className="flex flex-col items-start gap-2.5">
          <div className="flex items-center gap-2 py-[0.5rem] px-[1.77rem] rounded-lg bg-[var(--form-primary)]">
            <span className="text-white text-sm font-medium leading-6">Renewal date: 10 Mar 2026</span>
          </div>
        </div>

        <div className="flex justify-center">
          <button className="py-[0.61rem] px-[4.72rem] rounded-lg bg-[var(--form-secondary)] text-[#0A0A0A] text-sm font-medium leading-6 whitespace-nowrap">
            Manage the Subscription
          </button>
        </div>

        {/* ── 2. Pricing Plans ── */}
        <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-4 gap-[1.33rem]">
          {plans.map((plan) => (
            <div key={plan.id} className={cardClass(plan.cardStyle)}>
              ...
              <div className="p-[1.33rem] flex flex-col h-full">
                ...
                <div className="flex items-end gap-2 mb-2">
                  {plan.priceUnit ? (
                    ...
                    <span
                      ...
                      style={{ fontFamily: "'Inter', sans-serif", fontSize: '1.44rem', lineHeight: '36px' }}
                    >
                      {plan.price}
                    </span>
                  )}
                ...
                {/* Description */}
                <p className="text-[#6B7280] text-xs leading-[22px] mb-4 min-h-[2.44rem]">{plan.desc}</p>

                {/* Credits pill */}
                <div className="flex items-center justify-between px-3 py-[0.72rem] rounded-lg bg-[#FFEAE8] mb-3">
                  <span className="text-[#374151] text-[0.66rem] font-medium leading-tight">Monthly Credits Included</span>
                  <span className="text-[#4B5563] text-[0.83rem] font-bold leading-none shrink-0">{plan.credits}</span>
                </div>
        ...
        <div className="flex flex-col gap-3 p-2 rounded-2xl bg-[#F6F6F6]">
          ...
          <button onClick={() => openModal("downgrade")} className="flex items-center justify-center py-[0.72rem] px-[1.77rem] rounded-lg bg-white w-full">
            <span className="text-black font-medium text-base" style={{ fontFamily: "'Manrope', sans-serif" }}>Downgrade plan</span>
          </button>
          <button onClick={() => openModal("freeze")} className="flex items-center justify-center py-[0.72rem] px-[1.77rem] rounded-lg bg-white w-full">
            <span className="text-black font-medium text-base" style={{ fontFamily: "'Manrope', sans-serif" }}>Freeze for 1 month (Travel mode)</span>
          </button>
          <button onClick={() => openModal("cancel")} className="flex items-center justify-center py-[0.72rem] px-[1.77rem] rounded-lg bg-white w-full">
            <span className="text-black font-medium text-base" style={{ fontFamily: "'Manrope', sans-serif" }}>Cancel subscription anytime</span>
          </button>
        </div>

        {/* ── 4. Available Credits ── */}
        <div className="rounded-[0.77rem] border-2 border-[var(--form-primary)] bg-[rgba(248,131,121,0.12)] p-8">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-[1.33rem]">
            {/* Left side */}
            <div className="flex flex-col gap-2">
              ...
              <div className="flex items-center gap-3">
                ...
                <span
                  ...
                  style={{ fontFamily: "'Manrope', sans-serif", fontSize: "2.66rem", lineHeight: "2.66rem" }}
                >
                  156
                </span>
              </div>
            ...
            <div className="flex flex-col items-stretch sm:items-end gap-2 w-full sm:w-auto">
              <button className="flex items-center justify-center gap-2 py-2 px-4 rounded-lg bg-[var(--form-secondary)] h-[2.33rem] whitespace-nowrap">
                <PlusIcon />
                <span className="text-black text-sm font-normal" style={{ fontFamily: "'Poppins', sans-serif" }}>Buy More Credits</span>
              </button>
              <button className="flex items-center justify-center py-[0.5rem] px-[1.77rem] rounded-lg bg-[var(--form-primary)] h-[2.66rem] whitespace-nowrap">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-[1.33rem]">
            {creditPackages.map((pkg, i) => (
              <div
                key={i}
                className={
                  pkg.popular
                    ? 'relative rounded-[0.66rem] border-2 border-[#7FFFD4] bg-gradient-to-br from-[rgba(127,255,212,0.20)] to-[rgba(255,255,255,0.30)] flex flex-col items-center pt-8 pb-8 px-4'
                    : 'relative rounded-[0.66rem] border-2 border-[#E5E7EB] bg-white flex flex-col items-center pt-6 pb-6 px-4'
                }
              >
                {/* Most popular badge */}
                {pkg.popular && (
                ...
                {/* Icon */}
                <div className="flex items-center justify-center w-[3.11rem] h-[3.11rem] rounded-full bg-[var(--form-primary)] mb-4 flex-shrink-0">
                  <CreditIcon28White />
                </div>
                ...
                {/* Button */}
                {pkg.popular ? (
                  <button className="w-full py-[0.61rem] px-4 rounded-lg bg-[var(--form-primary)] text-white text-sm font-medium text-center">
                    Purchase Pack
                  </button>
                ) : (
        ...
        {/* ── 6. Usage History ── */}
        <div className="rounded-[0.77rem] border border-black/10 bg-white flex flex-col gap-8 p-[1.33rem]">
          ...
          <div className="flex flex-col gap-3">
            {usageHistory.map((item, i) => (
              <div
                key={i}
                className="flex items-center justify-between px-4 py-5 rounded-[0.55rem]"
              >
                {/* Left: icon + text */}
                <div className="flex items-center gap-4 min-w-0">
                  {/* Icon circle */}
                  <div
                    className={`flex items-center justify-center w-[2.22rem] h-[2.22rem] rounded-full flex-shrink-0 ${
        ...
        {activeModal === "cancel" && (
          <CancelModal onClose={closeAllModals} />
        )}

        {activeModal === "freeze" && (
          <FreezeModal onClose={closeAllModals} currentPlanPrice={149} />
        )}

        {activeModal === "downgrade" && (
          <DowngradeModal onClose={closeAllModals} />
        )}
