...
export default function DowngradeModal({ onClose, onDowngrade }: DowngradeModalProps) {
  return (
      ...
      >
        ...
        <div>
          <h2
            ...
            style={{ fontSize: "1.33rem", lineHeight: "150%" }}
          >
        ...
        {/* Info box */}
        <div
          className="flex items-start gap-[0.55rem] rounded-lg p-2 overflow-hidden"
          style={{ background: "rgba(56, 191, 65, 0.10)" }}
        >
          <InfoIcon />
          <p
            className="font-poppins font-normal flex-1"
            style={{ fontSize: "0.66rem", color: "#575757", lineHeight: "150%" }}
          >
        ...
        <button
          onClick={onDowngrade}
          className="flex items-center justify-center gap-[0.55rem] rounded-lg bg-white font-poppins font-normal text-sm text-[#0A0A0A] text-center transition-opacity hover:opacity-80"
          style={{ width: "100%", height: "2.66rem", padding: "1.33rem" }}
        >
          Downgrade Plan
        </button>
      </div>
